...
import { pagePath } from '@/lib/routing';
...
export async function pageMetadata(locale: string, slug: string): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${origin}${pagePath(locale, slug)}`, languages: Object.fromEntries(page.translations.map((item) => [item.locale, `${origin}${pagePath(item.locale, item.slug)}`])) },
  };
}
  ...
  return (
    <div data-theme={site.settings.theme ?? 'rose'} className="mx-auto flex min-h-[100dvh] max-w-7xl flex-col px-6 sm:px-10 lg:px-16">
      ...
      <header className="flex flex-wrap items-center justify-between gap-5 border-b border-line py-7 sm:py-9">
        <Link href={`/${locale}`} className="text-lg font-semibold tracking-tight hover:text-primary sm:text-xl">{site.name}</Link>
        <nav aria-label={nav('language')} className="flex items-center gap-2 text-sm">
          {page.translations.map((item) => <Link key={item.locale} href={pagePath(item.locale, item.slug)} hrefLang={item.locale} lang={item.locale} aria-current={item.locale === locale ? 'page' : undefined} className={`px-3 py-3 hover:text-primary ${item.locale === locale ? 'font-bold underline decoration-primary underline-offset-8' : 'text-muted'}`}>{item.locale === 'vi' ? 'Tiếng Việt' : 'English'}</Link>)}
        </nav>
      </header>
      <main id="content" className="flex-1 py-20 sm:py-28 lg:py-36">
        ...
        {slug !== 'home' ? <Link className="mt-10 inline-flex min-h-12 items-center border-b border-primary font-semibold text-primary" href={`/${locale}`}>{copy('back')} <span aria-hidden="true" className="ml-5">↗</span></Link> : (site.settings.navigation?.[locale] ?? []).map((item) => <Link key={item.slug} className="mt-10 mr-8 inline-flex min-h-12 items-center border-b border-primary font-semibold text-primary" href={pagePath(locale, item.slug)}>{item.label} <span aria-hidden="true" className="ml-5">↗</span></Link>)}
      </main>
