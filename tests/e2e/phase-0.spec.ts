...
test('Vietnamese home renders DB content and SEO', async ({ page }) => {
  ...
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', 'http://kimmyphungmakeup.localhost:3000/vi');
...
test('language navigation retains the translated About page', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/vi\/gioi-thieu$/);
  await page.getByRole('link', { name: 'English', exact: true }).click();
  await expect(page).toHaveURL(/\/en\/about$/);
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('It starts with you.');
  await expect(page.locator('html')).toHaveAttribute('lang', 'en');
  await expect(page.locator('link[hreflang="vi"]')).toHaveAttribute('href', 'http://kimmyphungmakeup.localhost:3000/vi/gioi-thieu');
...
test('demo domain uses its own content and actual available translations', async ({ page }) => {
  ...
  await expect(page).toHaveURL('http://demo.localhost:3000/vi');
  ...
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', 'http://demo.localhost:3000/vi');
});
...
for (const path of ['/fr', '/vi/missing', '/en/gioi-thieu', '/vi/page.with.dots']) {
...
test('forged tenant and forwarded-host headers cannot select a different site', async ({ request }) => {
  ...
  const response = await request.get('http://localhost:3000/vi', { headers: { host: 'demo.localhost:3000', 'x-site-id': kimmy.id, 'x-forwarded-host': 'kimmyphungmakeup.localhost:3000' } });
...
test('concurrent tenant requests do not contaminate cached content', async ({ request }) => {
  await Promise.all(Array.from({ length: 12 }, async (_, i) => {
    const demo = i % 2 === 0;
    const response = await request.get('http://localhost:3000/vi', { headers: { host: `${demo ? 'demo' : 'kimmyphungmakeup'}.localhost:3000` } });
    expect(response.status()).toBe(200);
